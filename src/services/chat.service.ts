...
interface StartConversationInput {
  // widgetId is validated at runtime but optional at type level to match callers
  widgetId?: string;
  visitorId?: string;
  visitorName?: string;
  visitorEmail?: string;
...
export async function startConversation(input: StartConversationInput) {
  if (!input.widgetId) {
    throw new ChatError("widgetId is required", 400);
  }
...
interface SendMessageInput {
  // conversationId is validated at runtime but optional at type level to match callers
  conversationId?: string;
  content?: string;
  role?: "USER" | "AGENT";
  senderId?: string;
...
export async function sendMessage(input: SendMessageInput) {
  if (!input.conversationId) {
    throw new ChatError("conversationId is required", 400);
  }
  if (!input.content) {
    throw new ChatError("content is required", 400);
  }
