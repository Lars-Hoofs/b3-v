...
interface CreateKnowledgeBaseInput {
  // workspaceId is validated at runtime but optional at type level to match callers
  workspaceId?: string;
  name?: string;
  description?: string;
...
export async function createKnowledgeBase(input: CreateKnowledgeBaseInput) {
  if (!input.workspaceId) {
    throw new KnowledgeBaseError("workspaceId is required", 400);
  }
  if (!input.name) {
    throw new KnowledgeBaseError("name is required", 400);
  }
...
interface CreateDocumentInput {
  // knowledgeBaseId is validated at runtime but optional at type level to match callers
...
export async function createDocument(input: CreateDocumentInput) {
  if (!input.knowledgeBaseId) {
    throw new KnowledgeBaseError("knowledgeBaseId is required", 400);
  }
  if (!input.title) {
    throw new KnowledgeBaseError("title is required", 400);
  }
  if (!input.content) {
    throw new KnowledgeBaseError("content is required", 400);
  }
