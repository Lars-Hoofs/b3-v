...
interface CreateAgentInput {
  // Mark as optional at type level to satisfy callers, but validate at runtime
  workspaceId?: string;
  name?: string;
  description?: string;
  avatarUrl?: string;
...
export async function createAgent(input: CreateAgentInput) {
  if (!input.workspaceId) {
    throw new AgentError("workspaceId is required", 400);
  }
  if (!input.name) {
    throw new AgentError("name is required", 400);
  }
  if (!input.systemPrompt) {
    throw new AgentError("systemPrompt is required", 400);
  }
